...
const Hero = (props: {data: {headline: String, description: String, img: String}}) => {
    ...
    const textColor = templateFontColor();
    return (
        <section>
    <div className="grid max-w-screen-xl px-4 py-8 mx-auto lg:gap-8 xl:gap-0 lg:py-16 lg:grid-cols-12">
        <div className="mr-auto place-self-center lg:col-span-7">
            <h1 className="max-w-2xl mb-4 text-4xl font-extrabold tracking-tight leading-none md:text-5xl xl:text-6xl dark:text-white">{headline}</h1>
            <p className={`max-w-2xl mb-6 font-light text-${textColor} lg:mb-8 md:text-lg lg:text-xl`}>{description}</p>
            <Link href="/onboard" className={`inline-flex items-center justify-center px-5 py-3 mr-3 text-base font-medium text-center text-white rounded-lg bg-${buttonColor}-700 hover:bg-${buttonColor}-800 focus:ring-4 focus:ring-${buttonColor}-300 dark:focus:ring-${buttonColor}-900`}>
                {ctaLabel}
